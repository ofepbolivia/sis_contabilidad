...
<script>
  Phx.vista.FormFiltroDiario = Ext.extend(Phx.frmInterfaz, {
    ...
    constructor: function(config) {
      var storeTR = new Ext.data.ArrayStore({
        autoDestroy: true,
      ...
      this.Cmp.tipo_moneda.setValue('1');
      this.Cmp.estado_reg.setValue('validado,verificado,aprobado');
      this.Cmp.tipo_formato.store = storeTR;
    },
    iniciarEventos: function() {
      this.Cmp.id_gestion.on('select',function(c,r,n){
          console.log("gtesion -->", this.Cmp.id_gestion.getRawValue(), r.data.id_gestion);
          this.Cmp.desde.setMinValue('01/01/'+this.Cmp.id_gestion.getRawValue());
          this.Cmp.desde.setMaxValue('31/12/'+this.Cmp.id_gestion.getRawValue());
          this.Cmp.hasta.setMinValue('01/01/' + this.Cmp.id_gestion.getRawValue());
          this.Cmp.hasta.setMaxValue('31/12/'+this.Cmp.id_gestion.getRawValue());
          this.Cmp.desde.setValue('01/01/'+this.Cmp.id_gestion.getRawValue());
          this.Cmp.hasta.setValue('31/12/'+this.Cmp.id_gestion.getRawValue());
      },this);

      this.Cmp.tipo_diario.on('select', function() {
        if (this.Cmp.tipo_diario.getValue() == 'dia') {
          var storeTR = new Ext.data.ArrayStore({
            autoDestroy: true,
            fields: ['variable', 'valor'],
            data: [
          ...
          this.Cmp.tipo_formato.reset();
          this.Cmp.tipo_formato.store = storeTR;
          this.Cmp.tipo_formato.setValue('xls');
        } else if (this.Cmp.tipo_diario.getValue() == 'det') {
          var storeTR = new Ext.data.ArrayStore({
            autoDestroy: true,
            fields: ['variable', 'valor'],
            data: [
          ...
          this.Cmp.tipo_formato.reset();
          this.Cmp.tipo_formato.store = storeTR;
          this.Cmp.tipo_formato.setValue('pdf');
        }
      }, this);
    },
    ...
    getValues: function() {
      var data = {
        ...
        anio_gestion: this.Cmp.id_gestion.getRawValue(),        
        query_filter: [{
            name: 'id_gestion',
            type: 'numeric',
            field: 'incbte.id_gestion',
            value: this.Cmp.id_gestion.getValue(),
            comparison: 'eq'
          },
          {
            name: 'tipo_moneda',
            type: 'numeric',
            field: 'incbte.id_moneda',
            value: this.Cmp.tipo_moneda.getValue(),
            comparison: 'eq'
          },
          {
            name: 'desde',
            type: 'date',
            field: 'incbte.fecha',
            value: this.Cmp.desde.getValue() ? this.Cmp.desde.getValue().dateFormat('Y-m-d') : '',
            comparison: 'gte'
          },
          {
            name: 'hasta',
            type: 'date',
            field: 'incbte.fecha',
            value: this.Cmp.hasta.getValue() ? this.Cmp.hasta.getValue().dateFormat('Y-m-d') : '',
            comparison: 'lte'
          },
          {
            name: 'nro_cbte',
            type: 'string',
            field: 'incbte.nro_tramite#incbte.nro_cbte',
            value: this.Cmp.nro_cbte.getValue(),
            comparison: 'like'
          },
          {
            name: 'id_clase_comprobante',
            type: 'numeric',
            field: 'incbte.id_clase_comprobante',
            value: this.Cmp.id_clase_comprobante.getValue(),
            comparison: 'eq'
          },
          {
            name: 'estado_reg',
            type: 'list',
            field: 'incbte.estado_reg',
            value: this.Cmp.estado_reg.getValue(),
            comparison: 'in'
          }
        ]
      };
