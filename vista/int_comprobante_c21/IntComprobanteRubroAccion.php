...
/**
 *@package BoA
 *@file IntComprobanteRubroAccion.php
 *@author franklin.espinoza
 *@date 10-12-2021
 *@description  Vista para consulta de Comprobantes C21
 */
...
<script>
    Phx.vista.IntComprobanteRubroAccion=Ext.extend(Phx.gridInterfaz,{
        viewConfig: {
            stripeRows: false,
        ...
        constructor: function(config) {
            ...
            Phx.vista.IntComprobanteRubroAccion.superclass.constructor.call(this,config);

            this.store.baseParams.estado_cbte = 'borrador';

            this.addButton('btnImprimir', {
                text: '<b style="color:#00B167;">IMPRIMIR</b>',
                grupo: [0, 1, 2, 3, 4],
                iconCls: 'bprint',
                disabled: true,
            ...
            this.addButtonIndex(7, 'btn_cbte_erp', {
                iconCls: 'bsee',
                disabled: true,
                xtype: 'splitbutton',
                grupo: [0, 1,2,3,4],
                tooltip: '<b>Acciones para validar y desvalidar, comprobante C21 ERP.</b>',
                text: '<b style="color:#00B167;">EVENTOS ERP</b>',
                ...
                menu: [
                    {
                        text: '<b style="color: red">Validar CBTE C21</b>',
                        iconCls: 'bver-sigep',
                        argument: {
                            news: true,
                            action: 'validar',
                    ...
                    {
                        text: '<b style="color: red">Desvalidar CBTE C21</b>',
                        iconCls: 'bdes-sigep',
                        argument: {
                            news : true,
                            action: 'desvalidar',
                    ...
                    {
                        text: '<b style="color: red">Revertir CBTE C21</b>',
                        iconCls: 'brev-sigep',
                        argument: {
                            news: true,
                            action: 'revertir',
                    ...
                    {
                        text: '<b style="color: red">Clonar CBTE C21</b>',
                        iconCls: 'bnew-sigep',
                        argument: {
                            news: true,
                            action: 'clonar',
            ...
            this.addButtonIndex(8,'btn_cbte_sigep', {
                iconCls: 'bsee',
                xtype: 'splitbutton',
                grupo: [0, 1,2,3,4],
                tooltip: '<b>Acciones para procesar SIGEP</b>',
                text: '<b style="color:#00B167;">EVENTOS SIGEP</b>',
                argument: {
                    'news': true,
                    def: 'reset'
                },
                ...
                menu: [
                    {
                        text: '<b style="color: red;">Desverificar C21</b>',
                        iconCls: 'bdes-sigep',
                        argument: {
                            'news': true,
                            def: 'csv'
            ...
            this.getBoton('btn_cbte_erp').on('menushow', function(btn, e) {
                let record = this.getSelectedData();
                (btn.menu.items.items).forEach(function(item) {
                    if ( record.estado_reg == 'aprobado' ){
                        console.log('item', item);
                        if ( item.argument.action == 'revertir' || item.argument.action == 'clonar' ){
                            item.enable();
                        }else{
                            item.disable();
                        }
                    }else if(record.estado_reg == 'borrador') {
                        item.disable();
                    }else{
                        item.enable();
                    }
            ...
            this.getBoton('btn_cbte_sigep').on('menushow', function(btn, e) {
                let record = this.getSelectedData();
                (btn.menu.items.items).forEach(function(item) {
                    if ( record.estado_reg == 'borrador' || record.estado_reg == 'elaborado' || record.estado_reg == 'aprobado'){
                        item.disable();
                    }else{
                        item.enable();
                    }
        ...
        onValidar : function(){
            Ext.Msg.show({
                ...
                msg: '<b style="color: red;">Esta seguro de validar el comprobantes seleccionado.</b>',
                fn: function (btn){
                    if(btn == 'ok'){
                        Phx.CP.loadingShow();
                        let record = this.getSelectedData();
        ...
        onDesvalidar : function(){
            Ext.Msg.show({
                title: 'COMPROBANTE ERP',
                msg: '<b style="color: red;">Esta seguro de desvalidar el comprobante seleccionado.</b>',
                fn: function (btn){
                    if(btn == 'ok'){
        ...
        onRevertir: function(){
            ...
            Ext.Msg.show({
                title: 'REVERSIÓN COMPROBANTE',
                msg: '<b style="color: red;">Esta seguro de Revertir el comprobante seleccionado?.</b>',
                fn: function (btn){
                    if(btn == 'ok'){
                        var rec = this.getSelectedData();
                        Phx.CP.loadingShow();
                        Ext.Ajax.request({
                            url: '../../sis_contabilidad/control/IntComprobante/revertirCBTEC21',
                            params: {
        ...
        onClonar: function(){
            Ext.Msg.show({
                ...
                msg: '<b style="color: red;">Esta seguro de Clonar el comprobante seleccionado.</b>',
                fn: function (btn){
                    if(btn == 'ok'){
        ...
        preparaMenu : function(n) {
            var tb = Phx.vista.IntComprobanteRubroAccion.superclass.preparaMenu.call(this,n);
            var rec=this.sm.getSelected();
            this.getBoton('btnImprimir').enable();
            this.getBoton('btn_cbte_erp').enable();
            this.getBoton('btn_cbte_sigep').enable();
            return tb;
        },
        liberaMenu : function() {
            var tb = Phx.vista.IntComprobanteRubroAccion.superclass.liberaMenu.call(this);
            this.getBoton('btnImprimir').disable();
            this.getBoton('btn_cbte_erp').disable();
            this.getBoton('btn_cbte_sigep').disable();
        },
        ...
        desverificaProcesoSigep : function (wizard,response){
            var record = this.getSelectedData();
            Ext.Ajax.request({
                url:'../../sis_sigep/control/SigepActionC21/processChangeStates',
                params:{
                    id_service_request : record.id_service_request,
                    estado_reg : record.estado_reg,
                    momento : 'pass',
                ...
                success: function (resp) {
                    var reg =  Ext.decode(Ext.util.Format.trim(resp.responseText));
                    var datos = reg.ROOT.datos; console.log('desverificar C21', datos);
                    if(datos.process){
                        Phx.CP.loadingHide();
                        Ext.Ajax.request({
                            url:'../../sis_contabilidad/control/IntComprobante/anteriorEstado',
