CREATE OR REPLACE FUNCTION conta.f_gen_comprobante_pvr (
...
BEGIN
    ...
    v_nombre_funcion:='conta.f_gen_comprobante_pvr';
	...
	v_columnas=replace(v_columnas,'}','');
    v_consulta = 'select '||v_columnas ||' from '||v_plantilla.tabla_origen|| ' where '||v_plantilla.tabla_origen||'.'||v_plantilla.id_tabla||'='||p_id_tabla_valor||'';
    ...
    --tes.vrefrigerio_viatico_plan_pago
    execute	'select '||v_columnas ||' from '||v_plantilla.tabla_origen|| ' where '||v_plantilla.tabla_origen||'.'||v_plantilla.id_tabla||'='||p_id_tabla_valor||'' into v_tabla;
    ...
    if ( v_plantilla.campo_subsistema != ''  AND  v_plantilla.campo_subsistema is not null ) then
        v_this.columna_subsistema = conta.f_get_columna('maestro',v_plantilla.campo_subsistema::text,hstore(v_this),hstore(v_tabla));
	end if;

    --guardo depto
    if (v_plantilla.campo_depto != '' AND v_plantilla.campo_depto is not null) then
        v_this.columna_depto = conta.f_get_columna(	'maestro',v_plantilla.campo_depto::text,hstore(v_this),hstore(v_tabla));
	end if;

    if (v_plantilla.campo_depto_libro != '' AND v_plantilla.campo_depto_libro is not null) then
        v_this.columna_libro_banco = conta.f_get_columna(	'maestro',v_plantilla.campo_depto_libro::text,hstore(v_this),hstore(v_tabla));
	end if;

    --guardo acreedor
    if (v_plantilla.campo_acreedor != '' AND v_plantilla.campo_acreedor is not null) then
    	v_this.columna_acreedor = conta.f_get_columna(	'maestro',v_plantilla.campo_acreedor::text,hstore(v_this),hstore(v_tabla));
	end if;

    --guardo descripcion--
    if (v_plantilla.campo_descripcion != '' AND v_plantilla.campo_descripcion is not null) then
    	v_this.columna_descripcion = conta.f_get_columna(	'maestro',v_plantilla.campo_descripcion::text,hstore(v_this),hstore(v_tabla));
	end if;

    --guardo moneda--
    if (v_plantilla.campo_moneda != '' AND v_plantilla.campo_moneda is not null) then
    	v_this.columna_moneda = conta.f_get_columna(	'maestro',v_plantilla.campo_moneda::text,hstore(v_this),hstore(v_tabla))::integer;
	end if;

    --guardo fecha
    if (v_plantilla.campo_fecha != '' AND v_plantilla.campo_fecha is not null) then
    	v_this.columna_fecha = conta.f_get_columna(	'maestro',v_plantilla.campo_fecha::text,hstore(v_this),hstore(v_tabla));
	end if;

    --guardo campo_fecha_costo_ini
    if (v_plantilla.campo_fecha_costo_ini != '' AND v_plantilla.campo_fecha_costo_ini is not null) then
    	v_this.columna_fecha_costo_ini = conta.f_get_columna(	'maestro',v_plantilla.campo_fecha_costo_ini::text,hstore(v_this),hstore(v_tabla));
	end if;

    --guardo campo_fecha_costo_fin
    if (v_plantilla.campo_fecha_costo_fin != '' AND v_plantilla.campo_fecha_costo_fin is not null) then
    	v_this.columna_fecha_costo_fin = conta.f_get_columna(	'maestro',v_plantilla.campo_fecha_costo_fin::text,hstore(v_this),hstore(v_tabla));
	end if;

      --guardo gestion_relacion
    if (v_plantilla.campo_gestion_relacion != '' AND v_plantilla.campo_gestion_relacion is not null) then
        v_this.columna_gestion = conta.f_get_columna('maestro',v_plantilla.campo_gestion_relacion::text,hstore(v_this),hstore(v_tabla))::integer;
	end if;
    ...
    if ( v_plantilla.campo_id_cuenta_bancaria != ''  AND  v_plantilla.campo_id_cuenta_bancaria is not null ) then
        v_this.columna_id_cuenta_bancaria = conta.f_get_columna('maestro',v_plantilla.campo_id_cuenta_bancaria::text,hstore(v_this),hstore(v_tabla));
	end if;
    ...
    if ( v_plantilla.campo_id_cuenta_bancaria_mov != ''  AND  v_plantilla.campo_id_cuenta_bancaria_mov is not null ) then
        v_this.columna_id_cuenta_bancaria_mov = conta.f_get_columna('maestro',v_plantilla.campo_id_cuenta_bancaria_mov::text,hstore(v_this),hstore(v_tabla));
	end if;

     --JRR: guardar campo_nro_cheque
    if ( v_plantilla.campo_nro_cheque != ''  AND  v_plantilla.campo_nro_cheque is not null ) then
        v_this.columna_nro_cheque = conta.f_get_columna('maestro',v_plantilla.campo_nro_cheque::text,hstore(v_this),hstore(v_tabla));
	end if;

     --RCM: guardar campo_nro_tramite
    if ( v_plantilla.campo_nro_tramite != ''  AND  v_plantilla.campo_nro_tramite is not null ) then
        v_this.columna_nro_tramite = conta.f_get_columna('maestro',v_plantilla.campo_nro_tramite::text,hstore(v_this),hstore(v_tabla));
	end if;
    ...
    if ( v_plantilla.campo_nro_cuenta_bancaria_trans != ''  AND  v_plantilla.campo_nro_cuenta_bancaria_trans is not null ) then
        v_this.columna_nro_cuenta_bancaria_trans = conta.f_get_columna('maestro',v_plantilla.campo_nro_cuenta_bancaria_trans::text,hstore(v_this),hstore(v_tabla));
	end if;


    --RAC: guardar campo_tipo_cambio
    if ( v_plantilla.campo_tipo_cambio != ''  AND  v_plantilla.campo_tipo_cambio is not null ) then
        v_this.columna_tipo_cambio = conta.f_get_columna('maestro',v_plantilla.campo_tipo_cambio::text,hstore(v_this),hstore(v_tabla))::numeric;
	end if;
    ...
    if ( v_plantilla.campo_cbte_relacionado != ''  AND  v_plantilla.campo_cbte_relacionado is not null ) then
        v_this.columna_cbte_relacionado = conta.f_get_columna('maestro',v_plantilla.campo_cbte_relacionado::text,hstore(v_this),hstore(v_tabla))::Varchar;
	end if;

    v_resp:=v_this;

    --raise 'v_this: %',v_this.columna_fecha;
    --forzamos que la fecha se quede en los limites de la gestion
    v_gestion_fecha =  date_part('year', v_this.columna_fecha);

    select g.gestion into v_gestion_aux
...
--raise 'llega bolivia';
    ...
    IF v_plantilla.codigo_tipo_relacion is not null and  v_plantilla.codigo_tipo_relacion != ''   THEN
       select c.id_tipo_relacion_comprobante into v_id_tipo_relacion_comprobante
   ...
   /*-------------------------------------------------------------------
    ...
    and fir.id_depto = v_this.columna_depto;
    /*raise 'columna_depto: %, columna_libro_banco: %, columna_descripcion: %, columna_acreedor: %, columna_fecha: %, columna_id_cuenta_bancaria: %,
        columna_id_cuenta_bancaria_mov: %, columna_nro_cheque: %, columna_nro_cuenta_bancaria_trans: %, columna_fecha_costo_ini: %, columna_fecha_costo_fin: %, columna_cbte_relacionado: %',v_this.columna_depto, v_this.columna_libro_banco,v_this.columna_descripcion, v_this.columna_acreedor, v_this.columna_fecha,v_this.columna_id_cuenta_bancaria,
                    v_this.columna_id_cuenta_bancaria_mov,v_this.columna_nro_cheque,v_this.columna_nro_cuenta_bancaria_trans,v_this.columna_fecha_costo_ini, v_this.columna_fecha_costo_fin,
                    v_this.columna_cbte_relacionado;*/
...
--raise 'hstore(v_plantilla): %',hstore(v_plantilla);
    resp_det =  conta.f_gen_transaccion_pvr(hstore(v_this),
                            hstore(v_tabla),
                            hstore(v_plantilla),
                            p_id_tabla_valor,
...
ALTER FUNCTION conta.f_gen_comprobante_pvr (p_id_tabla_valor integer, p_codigo varchar, p_id_estado_wf integer, p_id_usuario integer, p_id_usuario_ai integer, p_usuario_ai varchar, p_conexion varchar, p_sincronizar_internacional boolean, p_nombre_origen varchar)
  OWNER TO postgres;