	...
	***********************************/
		...
		begin
			...
			v_consulta:='select count(id_comisionista),
            			COALESCE(sum(monto_total),0)::numeric as total_monto_total,
						COALESCE(sum(monto_total_comision),0)::numeric as total_monto_total_comision
					    from conta.tcomisionistas cm
                        inner join segu.tusuario usu1 on usu1.id_usuario = cm.id_usuario_reg
                        inner join param.tperiodo per on per.id_periodo = cm.id_periodo
                        inner join param.tgestion ges on ges.id_gestion = per.id_gestion
    ...
    /*********************************
 	#TRANSACCION:  'CONTA_CMS_REPO'
 	#DESCRIPCION:	Reporte general de totales por agencias
 	#AUTOR:		MMV
 	#FECHA:		28/11/2017
	***********************************/

    elsif(p_transaccion='CONTA_CMS_REPO')then
