    ...
    --2) IF ... si la transaccion es de ajuste  no se hacen cambios (actualizacion = si)
     ...
     IF v_registros.actualizacion = 'no' THEN
           ...
           ELSE
             ...
             ELSE
             --localidad internacional para comprobantes que llegan a estacion central bolivia de cualquier estacion internacional
             	 v_valor_debe_mb =  param.f_convertir_moneda (v_registros.id_moneda, v_id_moneda_base,   v_registros.importe_debe, v_registros.fecha,'CUS',50, v_registros.tipo_cambio_2, 'no');
                 v_valor_haber_mb =  param.f_convertir_moneda (v_registros.id_moneda, v_id_moneda_base,   v_registros.importe_haber, v_registros.fecha,'CUS',50, v_registros.tipo_cambio_2, 'no');
...
ALTER FUNCTION conta.f_calcular_monedas_transaccion (p_id_int_transaccion integer)
  OWNER TO postgres;