...
class MODBancaCompraVenta extends MODbase
{
    ...
    function listarBancaCompraVenta()
    {
        ...
        $this->captura('tipo_documento_respaldo', 'varchar');
        $this->captura('nro_documento_respaldo', 'varchar');
    ...
    function insertarBancaCompraVenta()
    {
        ...
        $this->setParametro('tipo_documento_respaldo', 'tipo_documento_respaldo', 'varchar'); //fRnk: HR00586-2025
        $this->setParametro('nro_documento_respaldo', 'nro_documento_respaldo', 'varchar');
        //Ejecuta la instruccion
        $this->armarConsulta();
        $this->ejecutarConsulta();
    ...
    { //fRnk: varias modificaciones en bancarización para la importación y exportación, HR00586
        $arra = array();
                ...
                if (!empty(trim($line))) {
                    if ($cl == 0) {
                        if (substr_count($line, '|') > 10) {
                            $separador = '|';
                        } else if (substr_count($line, ',') > 10) {
                            $separador = ',';
                        }
                        $arr_temp = explode($separador, $line);
                        if ($this->aParam->getParametro('tipo') == 'Compras' && count($arr_temp) != 21) {
                            $mensaje_completo .= "Los datos del archivo no cumplen con el formato establecido.\n";
                            $this->mensajeRes = new Mensaje();
                            $this->mensajeRes->setMensaje('ERROR', 'ATCBancaCompraVenta.php', $mensaje_completo,
                                $mensaje_completo, 'control');
                            return $this->mensajeRes;
                            break;
                        } else if($this->aParam->getParametro('tipo') == 'Ventas' && count($arr_temp) != 19){
                            $mensaje_completo .= "Los datos del archivo no cumplen con el formato establecido.\n";
                            $this->mensajeRes = new Mensaje();
                            $this->mensajeRes->setMensaje('ERROR', 'ATCBancaCompraVenta.php', $mensaje_completo,
                                $mensaje_completo, 'control');
                        ...
                        if ($this->aParam->getParametro('tipo') == 'Compras') {
                            $arra[] = array(
                                'tipo_transaccion' => $arr_temp[1],
                                'modalidad_transaccion' => $arr_temp[2],
                                'nit_ci' => $arr_temp[3],
                                'complemento' => $arr_temp[4],
                                'razon' => $arr_temp[5],
                                'autorizacion' => $arr_temp[6],
                                'num_documento' => $arr_temp[7],
                                'tipo_documento_respaldo' => $arr_temp[8],
                                'nro_documento_respaldo' => $arr_temp[9],
                                'fecha_documento' => $arr_temp[10],
                                'importe_documento' => $arr_temp[11],
                                ...
                                'num_documento_pago' => $arr_temp[19],
                                'monto_pagado' => $arr_temp[20]
                            );
                            ...
                            $arra[] = array(
                                ...
                                'tipo_documento_pago' => $arr_temp[13],
                                'fecha_de_pago' => $arr_temp[14],
                                'num_cuenta_pago' => $arr_temp[15],
                                'nit_entidad' => $arr_temp[16], //
                                'nit_financiera_abono' => $arr_temp[16],
                                'num_documento_pago' => $arr_temp[17],
                                'monto_pagado' => $arr_temp[18]
                            );
                        }
                    }
                    $cl++;
